{
  ...
  generateFontFamily = builtins.map (family: {
    inherit family;
  });

  generateFontConfig = let
    toXML = {
      version ? "1.0",
      encoding ? null,
      standalone ? null,
      ...
      repeatString = string: count: optionalString (count > 0) (string + (repeatString string (count - 1)));
      ...
      documentTypeDefinition = optionalString (isNotNull dtd) (
        "<!DOCTYPE"
